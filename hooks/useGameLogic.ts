...
import { PlayerState, BirdInstance, BattleResult, GearType, Gear, Rarity, UpgradeState, Gem, ConsumableType, Consumable, Bird, APShopState, UnlocksState, ZoneClearReward, StatType, StatOption } from '../types';
...
import { loadGame, saveGame, resetGame, getLastActiveSlot } from '../utils/persistence';
import { generateStatOptions } from '../components/battle/utils';

export const useGameLogic = () => {
  const [activeSlot, setActiveSlot] = useState<number>(() => getLastActiveSlot());
  const [playerState, setPlayerState] = useState<PlayerState>(() => loadGame(getLastActiveSlot()));

  // Ref to track state for saving without triggering re-effects
                   ...
                   if (bird.id === 'hummingbird') {
                        ...
                        if (Math.random() < 0.25) {
                            ...
                            if (newXp !== bird.xp || newLevel !== bird.level) {
                                birdsUpdated = true;
                                const levelsGained = Math.max(0, newLevel - bird.level);
                                const updatedBird = { 
                                    ...bird, 
                                    xp: newXp, 
                                    level: newLevel, 
                                    xpToNextLevel: newXpToNext,
                                    statPoints: (bird.statPoints || 0) + levelsGained
                                };
                                if (updatedBird.statPoints > 0 && !updatedBird.pendingStatOptions) {
  ...
  const handleZoneSuccess = (reward: ZoneClearReward, clearedZoneNumber: number) => {
      setPlayerState(prev => {
          ...
          return {
              ...
              currentZoneProgress: isProgression ? [] : prev.currentZoneProgress, 
              inventory: newInventory,
              lifetimeStats: {
                  ...prev.lifetimeStats,
  ...
  const handleBattleComplete = (result: BattleResult, selectedZone: number): { pendingZoneUnlock: number | null, shouldAdvanceZone: boolean } => {
      ...
      setPlayerState(prev => {
          ...
          updatedBirds[birdIndex] = updatedBird;
  ...
  const handleApplyLevelUpReward = (birdId: string, stat: StatType, value: number) => {
      setPlayerState(prev => {
          ...
          if (bird.statPoints > 0) {
              ...
              // Regenerate options for the next point if available, otherwise clear
              if (bird.statPoints > 0) {
                  bird.pendingStatOptions = generateStatOptions();
              } else {
                  bird.pendingStatOptions = undefined;
              }
